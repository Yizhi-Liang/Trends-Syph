...
fit_yr_race = import(here("output", "stan_output", "1_main_P_alpha1_beta1.rds")) |>
  as_tibble() |>
  order_levels(c("year", "race")) |>
...
fit_yr_race |> 
  group_by(year, race) |> 
  summarise(
    P_alpha_mean = mean(P_alpha),
    P_alpha_lower = quantile(P_alpha, probs = 0.025),
    P_alpha_upper = quantile(P_alpha, probs = 0.975),
    P_beta_mean = mean(P_beta),
    P_beta_lower = quantile(P_beta, probs = 0.025),
    P_beta_upper = quantile(P_beta, probs = 0.975)
  )
...
fit_yr_race |> 
  ...
  summarise(
    ...
    Test_mean = mean(Test),
    Test_lower = quantile(Test, probs = 0.025),
    Test_upper = quantile(Test, probs = 0.975)
  )
